...
const firstSpecWithInjectedEnvironment = {
    @args({$ref: 'environment.data'})
    first: (data) => `first_${data.url}`
}

const specWithEnvironment = {
    @args()
    environment: () => ({
        data: {
            url: 'http://example.com'
    ...
    @defer(
        [firstSpecWithInjectedEnvironment],
        {$ref: 'environment'}
    )
    deferredComponent: {},
...
describe('Access to injected object fields by dot', async () => {
    ...
    before(async function() {
        try {
            context = await createContext(specWithEnvironment);
        } catch (error) {
            console.log('ERROR:' , error);
        }
    });
    ...
    it('context A should have component with value', () => {
        expect(context.A.first).to.equal('first_http://example.com');
    });
